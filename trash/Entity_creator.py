...
from .UI_classes import menu_creator , execute_file_module , desimal_inputfield , Sort_menus , catch_menus , ScrollableInputField
from ursina.prefabs.dropdown_menu import DropdownMenu, DropdownMenuButton
from .tree_button import entity_buttons
from ..Creator_methods.tree_projects_entity import rename_key_in_tree
...
class parent_panel():
    def __init__(self):
        self.panel_name = WindowPanel(enabled = False)
        self.panel_position = WindowPanel(enabled = False)
        self.panel_rotation = WindowPanel(enabled = False)
        self.panel_scale = WindowPanel(enabled = False)
        self.panel_color = WindowPanel(enabled = False)
    ...
    def window_panel_color(self , **func):
        ...
        print(f"Color picer type ==> {type(self.panel_color.content[-1].value)} ======> color : {self.panel_color.content[-1].value}")
